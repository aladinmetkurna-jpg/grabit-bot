...
def admin(message):
    ...
    markup.add(telebot.types.InlineKeyboardButton("⚙️ Limit ozgartirish", callback_data="set_limit"))
...
def cb(call):
    ...
    if call.data == "check_sub":
        ...
        bot.answer_callback_query(call.id)
        msg = bot.send_message(uid, "⚙️ Yangi limitni yozing:")
...
def download(message):
    ...
    if users[uid]["count"] >= LIMIT:
        if not check_sub(uid):
            bot.send_message(uid,
                "⚠️ Limitingiz tugadi!\n\n"
                "Davom etish uchun kanallarga obuna bоling:",
    ...
    try:
        ...
        users[uid]["count"] += 1
        left = LIMIT - users[uid]["count"]

        with open(filename, 'rb') as f:
            if filename.endswith(('.mp4', '.webm', '.mkv')):
                bot.send_video(uid, f, caption="✅ " + info.get('title', 'Video') + "\n📥 " + str(left) + " ta limit qoldi")
            else:
                bot.send_audio(uid, f, caption="✅ " + info.get('title', 'Audio') + "\n📥 " + str(left) + " ta limit qoldi")

        os.remove(filename)
        bot.delete_message(uid, msg.message_id)

    except Exception as e:
        bot.edit_message_text("❌ Xatolik: " + str(e), uid, msg.message_id)
