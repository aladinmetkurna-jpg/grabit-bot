...
MODEL_NAME = os.getenv("GLM_MODEL", "glm-5.3-flash")
GLM_BASE_URL = os.getenv("GLM_BASE_URL", "https://api.b.ai/v1")
...
# =========================================================
# GLM CLIENT (timeout bilan)
# =========================================================

client = OpenAI(
    api_key=GLM_API_KEY,
    base_url=GLM_BASE_URL,
    timeout=45.0,          # 45 soniyadan oshsa to'xtaydi
    max_retries=2          # avtomatik 2 marta qayta urinib ko'radi
)
...
Agar foydalanuvchi "Sen kimsan?", "Kimsan?", "Isming nima?" desa:
...
def ask_luna(messages):
    """Streaming bilan so'rov yuboradi, to'liq javobni yig'ib qaytaradi."""
    try:
        ...
        # Streaming ishlamasa oddiy usulga o'tamiz
        logger.warning("Streaming ishlamadi, oddiy usulga o'tilmoqda: %s", e)
...
def get_error_message(error):
    ...
    if any(x in error_text for x in ["401", "invalid api key", "invalid_api_key", "authentication", "unauthorized"]):
        return "GLM API key bilan muammo bor. Railway Variables'dagi GLM_API_KEY ni tekshir."
    ...
    if any(x in error_text for x in ["timeout", "timed out", "connection", "read timeout"]):
...
def process_message(message: Message, is_photo=False):
    ...
    with lock:
        try:
            ...
            # Typingni yangilab turish (sekin javobda foydali)
            bot.send_chat_action(user_id, "typing")
            ...
            logger.info(
                ...
                user_id, message.chat.type, is_photo
            )

        except Exception as e:
            logger.exception("LUNA ERROR | user=%s", user_id)

            if user_chats.get(user_id) and user_chats[user_id][-1].get("role") == "user":
                user_chats[user_id].pop()

            error_message = get_error_message(e)

            if is_group:
                bot.send_message(user_id, error_message, reply_to_message_id=message.message_id)
            else:
...
@bot.message_handler(
    content_types=["video", "audio", "document", "sticker", "voice", "animation", "contact", "location"]
)
def unsupported_message(message):
    bot.send_message(message.chat.id, "Hozircha faqat matn va rasm bilan ishlayman.")
